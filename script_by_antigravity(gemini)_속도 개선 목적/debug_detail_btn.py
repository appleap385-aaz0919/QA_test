        ...
        try:
            ...
            for i, btn in enumerate(all_buttons):
                try:
                    ...
                    # textContent (소스 텍스트)
                    text_content = await btn.evaluate("el => el.textContent")
                    # 클래스
                    class_name = await btn.get_attribute("class") or ""

                    # 단원, 상세, 보기 관련 버튼만 출력
                    if any(kw in inner_text for kw in ["단원", "상세", "보기", "재구성"]):
                        print(f"[{i+1}] innerText: '{inner_text}'")
            ...
            # 스크린샷
            await main_page.screenshot(path="screenshots/debug_detail_btn.png", full_page=True)
            print("\n   스크린샷: screenshots/debug_detail_btn.png")
