...
- LNB '교과서' 메뉴 > 단원 리스트 > 재구성 하기 버튼 클릭 > 임시저장 삭제(있으면) > 임시저장 > 배포
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if recon_btn_count > 0:
                ...
                page_text = await main_page.locator("body").inner_text()
                recon_keywords = ["재구성", "모듈", "차시", "저장", "미리보기", "배포"]
                ...
                if deploy_btn_count > 0 and deploy_btn:
                    ...
                    if modal_clicked:
                        print("   배포 완료")
                        await main_page.screenshot(path=f"{SCREENSHOT_DIR}/tc07_04_after_deploy.png", full_page=True)
                        results["checks"]["배포_확인"] = {"modal_clicked": modal_clicked, "status": "PASS"}
                        results["steps"].append({"step": 9, "action": "배포 확인 버튼 클릭", "status": "PASS"})
                    else:
                        print("   모달 확인 버튼을 찾을 수 없음")
                        results["checks"]["배포_확인"] = {"modal_clicked": False, "status": "CHECK"}
                        results["steps"].append({"step": 9, "action": "배포 확인 버튼 찾기", "status": "CHECK"})
                else:
                    print("   배포 버튼을 찾을 수 없음")
                    results["checks"]["배포_버튼"] = {"found": False, "status": "FAIL"}
                    results["steps"].append({"step": 8, "action": "배포 버튼 찾기", "status": "FAIL"})
            else:
            ...
            with open("test_result_TC-T-07.json", "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("TC-T-07: 재구성 배포 동작 확인 테스트 시작")
    print("=" * 60)
    asyncio.run(test_reconstruction_deploy())
