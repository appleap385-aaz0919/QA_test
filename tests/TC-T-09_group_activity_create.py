...
async def test_group_activity_create():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("=" * 60)
            group_menu_selectors = ["text=/모둠활동/", "text=/모둠 활동/", "text=/모둠/"]
            group_menu = None
            ...
            if write_btn:
                ...
                print("=" * 60)
                tile_selectors = ["label:has-text('타일형')", "span:has-text('타일형')", "[class*='radio']:has-text('타일')"]
                tile_selected = False
                ...
                results["steps"].append({"step": 7, "action": "타일형 선택 확인", "status": "PASS" if tile_selected else "CHECK"})
                ...
                if create_btn:
                    try:
                        ...
                        if has_sample or has_sample_content:
                            ...
                            results["checks"]["새창_sample"] = {"found": False, "url": activity_url, "status": "CHECK"}
                            results["steps"].append({"step": 8, "action": "만들기 버튼 클릭", "status": "PASS"})
                        ...
                        await main_page.screenshot(path=f"{SCREENSHOT_DIR}/tc09_04_after_delete.png", full_page=True)
