    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if recon_btn_count > 0:
                ...
                save_btn_selectors = ["button:has-text('임시저장')", "button:has-text('저장')", "span:has-text('임시저장')"]
                save_btn_count = 0
                save_btn = None
                ...
                if preview_btn_count > 0 and preview_btn:
                    try:
                        viewer_page = await wait_for_new_page(context, lambda: preview_btn.click(), MAX_WAIT * 1000)
                        print("   미리보기 버튼 클릭")
