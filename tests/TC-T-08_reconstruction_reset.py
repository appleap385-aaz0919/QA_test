...
async def test_reconstruction_reset():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            results["steps"].append({"step": 3, "action": "단원 리스트 확인", "status": "PASS" if has_chapter else "FAIL"})
            ...
            if recon_btn_count > 0:
                ...
                print("=" * 60)
                delete_selectors = ["button:has-text('임시저장 삭제')", "button:has-text('삭제')"]
                delete_btn_count = 0
                delete_btn = None
                ...
                if delete_btn_count > 0:
                    ...
                    results["steps"].append({"step": 6, "action": "임시저장 삭제", "status": "PASS"})
                else:
                    print("   임시저장 삭제 버튼 없음 - 건너뜀")
                    results["checks"]["임시저장_삭제"] = {"found": False, "status": "SKIP"}
                    results["steps"].append({"step": 6, "action": "임시저장 삭제 확인", "status": "SKIP"})
                ...
                reset_btn_selectors = ["button:has-text('초기화')", "button:has-text('단원 초기화')"]
                reset_btn_count = 0
                reset_btn = None
                ...
                if reset_btn_count > 0 and reset_btn:
                    ...
                    results["steps"].append({"step": 8, "action": "초기화 버튼 클릭", "status": "PASS"})

                    # Step 9: 모달 팝업에서 확인 버튼 클릭 (최대 30초 대기)
                    print("\n" + "=" * 60)
                    print("Step 9: 모달 팝업에서 확인 버튼 클릭 (최대 30초 대기)")
                    print("=" * 60)
                    modal_confirm_selectors = ["button:has-text('확인')", "button:has-text('초기화')"]
                    modal_clicked = False

                    # 모달 버튼이 나타날 때까지 최대 30초 대기
                    print("   모달 버튼 대기 중...")
                    for wait_sec in range(30):
                        for selector in modal_confirm_selectors:
                            try:
                                ...
                                if await modal_btn.is_visible(timeout=1000):
                                    await modal_btn.click()
                                    print(f"   모달 확인 버튼 클릭 (대기 {wait_sec + 1}초)")
                                    modal_clicked = True
                                    await asyncio.sleep(0.5)
                                    break
                            except:
                    ...
                    if modal_clicked:
                        ...
                        # 토스트 알림 확인 (최대 30초 대기, 0.1초 간격)
                        print("   토스트 알림 확인 중... (최대 30초 대기, 0.1초 간격)")
                        toast_selectors = [
                            ".toast", "[class*='toast']", "[class*='Toast']",
                            ".ant-message", "[class*='message']", "[class*='notification']",
                            "[class*='alert']", "[class*='snackbar']", "[class*='popup']"
                        ]
                        toast_found = False
                        toast_message = ""

                        for check_count in range(300):  # 30초 / 0.1초 = 300회
                            # 첫 체크는 대기 없이 즉시 실행
                            if check_count > 0:
                                ...
                                try:
                                    ...
                                    if count > 0:
                                        for i in range(count):
                                            try:
                                                el = toast_elements.nth(i)
                                                if await el.is_visible(timeout=100):
                                                    toast_text = await el.inner_text()
                                                    # "초기화" 키워드가 포함된 토스트만 감지
                                                    if "초기화" in toast_text:
                                                        toast_found = True
                                                        toast_message = toast_text
                                                        print(f"   초기화 토스트 알림 발견: {toast_text}")
                                                        break
                                            except:
                                                continue
                                        if toast_found:
                        ...
                        if toast_found:
                            results["checks"]["초기화_확인"] = {"modal_clicked": modal_clicked, "toast_found": toast_found, "toast_message": toast_message, "status": "PASS"}
                            results["steps"].append({"step": 9, "action": "초기화 확인", "status": "PASS"})
                        else:
                            results["checks"]["초기화_확인"] = {"modal_clicked": modal_clicked, "toast_found": False, "status": "CHECK"}
                            results["steps"].append({"step": 9, "action": "초기화 확인", "status": "CHECK"})
                    else:
                        print("   모달 확인 버튼을 찾을 수 없음")
                        results["checks"]["초기화_확인"] = {"modal_clicked": False, "status": "CHECK"}
                        results["steps"].append({"step": 9, "action": "초기화 확인 버튼 찾기", "status": "CHECK"})
                else:
                    print("   초기화 버튼을 찾을 수 없음")
                    results["checks"]["초기화_버튼"] = {"found": False, "status": "FAIL"}
                    results["steps"].append({"step": 8, "action": "초기화 버튼 찾기", "status": "FAIL"})
            else:
